...
Pdata = genfromtxt('..\\Data\\Shiozaki et al selected.csv',delimiter=',').T #[:17] is for up to 150 (m)
figure(3)
plot(Pdata[3]/1000,Pdata[1],color='r')
plot(Pdata[3]/1000,Pdata[1],'o',color='r',markeredgecolor='k')

ylabel(Ylabel)
ylim(Ylim)
xlabel('PO$_4^{3-}$ ($\mu$mol kg$^{-1}$)')

PO4range = arange(0,0.3+0.01,0.01)
...
xlim(0.00,0.30)
ud(3)
Savefig3(Savefolder,'PO4Shiozaki',DPI)

figure(4)
plot(Pdata[2]/1000,Pdata[1],color='r')
plot(Pdata[2]/1000,Pdata[1],'o',color='r',markeredgecolor='k')

ylabel(Ylabel)
ylim(Ylim)
xlabel('NO$_3^{-}$ ($\mu$mol kg$^{-1}$)')

PO4range = arange(0,2.5+0.01,0.01)
y1 = zMax0*ones(size(PO4range))
y2 = zMax2*ones(size(PO4range))
fill_between(PO4range, y1, y2, where=y1 >= y2, facecolor='#FDD2D0',edgecolor = "none")
...
xlim(0.00,2.5)
ud(4)
Savefig3(Savefolder,'NO3Shiozaki',DPI)
