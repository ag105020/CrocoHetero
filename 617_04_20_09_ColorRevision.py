...
#   Ef=0.1
    ...
    figure(1)
    plot(RvA,ones(size(Vch)),dashes=(2.5,2.5),color='k',linewidth=2.5)
    plot(RvA,Vch/N0)
    xlabel('$f_N$')
    ylabel('$C_S$/$C_S^0$')
    ylim(0,1.20000001)
    Savefig2('HeteroC00\\HighRes14\\Revision','S0_Ef='+str(Ef),300)
...
#     ylabel('$C_S$/$C_S^0$')
#     #xlim(0,1)
#     Savefig2('HeteroC00\\HighRes14\\Revision','S1_Ef='+str(Ef),300)
    
    figure(3)  #Plot in fmol C cell-1 d-1
    Conversion = 1e15*3600 #from molC cell-1 s-1 to fmol cell-1 day-1
    stackplot(RvA,CCHbiomass*Conversion,CCHrespbiomass*Conversion,CCHN2fix*Conversion,CCHrespN2fix*Conversion,CCHextraresp*Conversion,colors=StackPolotColors)
    plot(RvA,Vch*Conversion,'k')
    #ylim(0,1.20000001)
    xlabel('$f_N$')
    ylabel('$C_S$ (fmol C cell$^{-1}$ h$^{-1}$)')
    #xlim(0,1)
    Savefig2('HeteroC00\\HighRes14\\Revision','S2_Ef='+str(Ef),600)
    return 
