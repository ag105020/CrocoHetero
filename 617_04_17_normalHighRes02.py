...
def Croco():
    ...
    #============================================================================
    Ef=0.2
    DdA=arange(0.001,1+0.001,0.001)
    Rv=1
    Urv=arange(size(DdA))
    o=zeros(size(DdA))
    ...
    title('$C_S$')
    xlabel(Xlab)
    ylabel('$C_S$ (mol C cell$^{-1}$ s$^{-1}$)')
    ylim(0,Ym)
    
    StackPolotColors=('#008000','red','#0000FF','#FFFF00','cyan')
    figure(2)
    stackplot(DdA,CCHbiomass,CCHN2fix,CCHrespbiomass,CCHrespN2fix,CCHextraresp,colors=StackPolotColors)
    ylim(0,Ym)
    title('$C_S$')
    xlabel(Xlab)
    ylabel('$C_S$ (mol C cell$^{-1}$ s$^{-1}$)')
    
    savetxt('VchHighRes02.csv', Vch, delimiter=",",fmt='%.8e')
    print(Vch)
