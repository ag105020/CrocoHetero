...
def Croco(a,Step,Min,Max):
    ...
    for i in Urv:
        ...
        #@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
        CCHbiomassF=ls/(-S4[1,8])*S4[1,0]                #(molC cell-1 s-1) consumption of CH for biomsas production
        CCHN2fixF=ls/(-S4[1,8])*S4[2,0]                  #(molC cell-1 s-1) consumption of CH for N2 fixation
        CCHbalancedrespF=ls/(-S4[1,8])*S4[0,0]           #(molC cell-1 s-1) consumption of CH for balanced respiration
...
tl=time.time()
for a in Array2:
   Vch, B1 = Croco(a,Step1,Min1,Max1)
...
figure(1)
pcolormesh(Array1,Array2,Box_Vch/Normal)
xlabel('$R_N$')
ylabel('$E_f$')
title('$C_S$')
xlim((Min1,1))
ylim((Min2,1))
xticks(arange(0.2,1.01,0.2))
yticks(arange(0.2,1.01,0.2))
colorbar()
clim(0,1)
plot(B1A,Array2,color='white')
Savefig('HeteroC00\\HighRes02',4)
Savetxt(Box_Vch/Normal,'HeteroC00\\HighRes02','01_Rn-Ef')
Savetxt(B1A,'HeteroC00\\HighRes02','01_B1A')
