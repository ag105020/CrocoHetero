...
def Croco(a,Step,Min,Max):
    ...
    # Creating a border
    #BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB
    Normal=Vch[-1]   #(mol C cell-1 s-1) When all the cells are nitrogen fixing (value from 617 04 13)
    EfVch = Vch/Normal
    
    for i in Urv:
        if i>0:
            if EfVch[i]<=1.00000001 and EfVch[i-1]>1:
                B1=RvA[i]
    
   # Vch = ma.masked_where(N2fixF > 6.1e-15,Vch)
    B1 = nan
...
B1B= Max1-argmin(Box_Vch[:,::-1],axis=1)/(Max1/Step1)     #here Argmin gives first min value but at Ef=1, there are multiple and I want it to point to the last min, so I change left and right of Box_Vch
...
#Normal=2.36016162e-18   #(mol C cell-1 s-1) When all the cells are nitrogen fixing (value from 617 04 13)
Normal=Vch[-1]   #(mol C cell-1 s-1) When all the cells are nitrogen fixing (value from 617 04 13)
print(Normal)
...
Savefig(SaveFolder,4)
Savetxt(Box_Vch,SaveFolder,'01_Rn-Ef')
Savetxt(B1A,SaveFolder,'01_B1A')
Savetxt(Box_N2fixF,SaveFolder,'01_N2FixF')
Savetxt(B1B,SaveFolder,'01_B1B')
Savetxt([Normal],SaveFolder,'01_Rn-Ef_Normal')
